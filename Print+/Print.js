...
define([
  ...
  var PrintDijit = declare([_WidgetBase, _TemplatedMixin, _WidgetsInTemplateMixin], {
    ...
    openAll: function() {
      var childDivs = this.printResultsNode.querySelectorAll('div > div');
      console.log('Number of child divs:', childDivs.length);

      // Initialize an empty string to store the merged output
      //var mergedOutput = '';
  
      for (var i = 0; i++ < childDivs.length; i++, i++, i++, i++) { // tot 20 kaarten
          console.log('Processing div', i + 1);
          childDivs[i].click(); // This will trigger the onclick event
          //break; // Exit after the first iteration

          // Append the content of the opened div to the merged output
          //mergedOutput += childDivs[i].innerText + '\n'; // You can adjust this as needed

          // Optional: Add a separator between each div's content
          //mergedOutput += '-----------------------------------\n';
  ...
  var printResultDijit = declare([_WidgetBase, _TemplatedMixin, _WidgetsInTemplateMixin], {
    ...
    _onPrintComplete: function(data) {
      if (data.url) {
        ...
        // Assign the modified URL
        //this.url = data.url;

       //var modifiedUrl = "MyFile" + data.url.substring(data.url.lastIndexOf("."));

        var modifiedUrl = data.url;
